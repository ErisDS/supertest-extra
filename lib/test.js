...
Test.prototype.assert = function(resError, res, fn) {
  ...
  if (resError instanceof Error && (resError.status >= 500 || resError.errno)) {
    error = resError;
    ...
    if (localError) {
      error = makeErrorFromObject(localError);
    }
  } else {
...
function makeErrorFromObject(localError) {
  var error = new Error(localError.message);
  error.type = localError.type || localError.errno || 'InternalServerError';
