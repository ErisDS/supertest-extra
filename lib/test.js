...
Test.prototype.assert = function(resError, res, fn) {
  ...
  if (resError instanceof Error && (resError.status >= 500 || !res)) {
    error = this._handlePreAssertError(resError, res);
  } else {
    // we can try to do our asserts
...
Test.prototype._handlePreAssertError = function (resError, res) {
  var localError;

  if (!res || resError.errno) {
    localError = resError;
  } else if (res.body.errors) {
    localError = res.body.errors[0];
  } else if (res.body.error) {
    localError = res.body.error;
  } else if (res.body.message) {
    localError = res.body;
  }

  if (localError) {
    return this._formatPreAssertError(localError);
  }

  return resError;
}

Test.prototype._formatPreAssertError = function(localError) {
  if (localError.errno === 'ECONNREFUSED') {
    // CASE: Connection to the server didn't work (URL was probably wrong)
    var msg = 'Connection Refused to: ' + this.url;
    var error = new Error(msg);
    error.stack = localError.stack.replace(/(Error:\s).*?(\n)/, '$1' + msg + '$2');
  } else {
    // CASE: catch all
    var error = new Error(localError.message);
    error.type = localError.type || 'InternalServerError';
    error.stack = localError.stack || {};
  }
